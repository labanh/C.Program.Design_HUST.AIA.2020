...
int refound_user(void) //重设密码界面
{
    ...
    prt_hz24(935, 12, "退出", 000000, "HZK\\Hzk24h");
    ...
    prt_hz24(259, 489, "重设密码：", 0, "HZK\\Hzk24h");
    ...
    while (1)
    {
        ...
        char key_0 = 0;                             //按下重设键后的状态，成功1，账号未输入2，密码未输入3，密码未确认4，身份证未输入5
        char key_1 = 0;                             //按下重设键后的状态，账号 存在0，不存在1
        char key_2 = 0;                             //按下重设键后的状态，密码格式，0表示长度不合乎要求，1表示合乎要求
        char key_3 = 0;                             //按下重设键后的状态，两次密码，0表示不相同，1表示相同
        char key_4 = 0;                             //按下重设键后的状态，身份证号码是否正确 0错误 1正确
        MouseShow(&mouse);                          //鼠标循环
        if (flag == 0 && MouseIn(895, 0, 1019, 50)) //退出框反馈
        {
            MouseOff(&mouse);
            Bar1(895, 0, 1019, 50, 56603);
            prt_hz24d(935, 12, "退出", 000000, "HZK\\Hzk24h");
            MouseOn(mouse);
            flag = 1;
        }
        if (flag == 0 && MouseIn(0, 0, 124, 50)) //返回框反馈
        {
            ...
            prt_hz24d(38, 13, "返回", 000000, "HZK\\Hzk24h");
            MouseOn(mouse);
            flag = 1;
        }
        ...
        if (MousePress(366, 614 + 100, 414, 638 + 100)) //点击确认键
        {
            ...
            if (key_0 == 1)
            {
                ...
                if (key_1 == 1)
                {
                    prt_hz24(670, 553, "账户未注册", 0, "HZK\\Hzk24h");
                    state = 0;
                }
            }
            /*判断密码格式*/
            ...
            if (key_3 == 1)
            {
                key_4 = right_id(username, id); //0表示不正确，1表示正确

                if (key_4 == 0)
                {
                    prt_hz24(670, 553 + 64, "身份证错误", 0, "HZK\\Hzk24h");
                    state = 0;
                }
            }
            if (state == 1)
            {
                user_change_password(username, key1);
                prt_hz24(670, 553, "密码重设成功", 0, "HZK\\Hzk24h");
                delay(1000);
                return 1;
            }
        }
